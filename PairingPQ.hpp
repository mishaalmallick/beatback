...
public:
    ...
    PairingPQ(const PairingPQ &other)
      ...
      if (other.root_ptr) {
        root_ptr = copyNode(other.root_ptr); 
        }
    ...
    PairingPQ &operator=(const PairingPQ &rhs) {
      if (this != &rhs) {
        ...
        if (rhs.root_ptr) {
            root_ptr = copyNode(rhs.root_ptr);  
        }
        else {
            root_ptr = nullptr;  
        }
...
Node* copyNode(Node* other_node) {
    ...
    Node* new_root = new Node(other_node->elt);
    
    std::deque<Node*> node_deque;  
    if(other_node->child){
        node_deque.push_back(other_node->child); 
    }


    while (!node_deque.empty()) {
        Node* current_original = node_deque.front();
        node_deque.pop_front(); 
        if(current_original->child){
            node_deque.push_back(current_original->child);
        }
        if(current_original->sibling){
            node_deque.push_back(current_original->sibling);
        }
        new_root = meld(new_root, current_original);

    }

    return new_root; // Return the root of the copied subtree
}
